...
const PullRequestEvent = ({ event }: { event: GitHubEvent }) => {
  ...
  return (
    <div className='flex items-center gap-4'>
      ...
      <div className='flex-1 truncate'>
        {action === 'closed' ? 'Merged' : 'Updated'} {pullRequest.user.login}
...
const IssuesEvent = ({ event }: { event: GitHubEvent }) => {
  ...
  return (
    <div className='flex items-center gap-4'>
      ...
      <div className='flex-1 truncate'>
        {event.actor.login} {action} {payload.issue.title} on {event.repo.name}
      </div>
      <EventDate date={event.created_at} />
    </div>
  )
...
const IssueCommentEvent = ({ event }: { event: GitHubEvent }) => {
  ...
  return (
    <div className='flex items-center gap-4'>
      ...
      <div className='flex-1 truncate'>
        Commented on {payload.issue.title} (#{payload.issue.number})
      </div>
      <EventDate date={event.created_at} />
    </div>
  )
...
const DeleteEvent = ({ event }: { event: GitHubEvent }) => {
  const refType =
    (event.payload as { ref_type?: 'branch' | 'tag' }).ref_type || 'branch'
...
const PullRequestReviewThreadEvent = ({ event }: { event: GitHubEvent }) => {
  ...
  return (
    <div className='flex items-center gap-4'>
      <GitPullRequestIcon className='h-4 w-4 shrink-0' />
      <div className='flex-1 truncate'>
        Marked a pull request thread as {action || 'resolved'} on{' '}
        {event.repo.name}
      </div>
      <EventDate date={event.created_at} />
    </div>
