...
Gorkie was built by [Devarsh](http://github.com/imdevarsh). I led the sandbox and runtime work; Devarsh helped throughout with testing and feedback.
...
We started with [Vercel Sandboxes](https://vercel.com/docs/vercel-sandbox/). We were already on [AI SDK](https://ai-sdk.dev/), so it was the obvious first choice. We gave Gorkie a sandbox tool that spawned a **ToolLoopAgent** to work inside it.
...
We switched to **[Daytona](https://www.daytona.io/)**. The model fit what we were building: each Slack thread owns a runtime, and that runtime persists across messages. Daytona's lifecycle settings made cleanup something the platform handled rather than us:
...
We next tried **[Pi](https://pi.dev/)** with Sandbox Agent. Pi gave us skills, MCPs, and easy extensibility, but the core shape was still an HTTP server inside the sandbox. Crashes still happened. We wanted simple/boring infrastructure.
