...
This blog post was a collaboration between two people. Gorkie itself was built by [Devarsh](http://github.com/imdevarsh), and I led the sandbox and runtime work, with Devarsh helping throughout with testing, and ideas.
...
We started with [Vercel Sandboxes](https://vercel.com/docs/vercel-sandbox/). An obvious choice since we were already on the [AI SDK](https://ai-sdk.dev/). We gave Gorkie a sandbox tool that spawned a **ToolLoopAgent** to work inside the sandbox.
...
This get-or-create with snapshot restore and Redis TTLs made per-thread persistence work before we had a proper DB model. On shutdown we would snapshot and save the ID with its own TTL:
...
It worked, but it was janky. Limited lifecycle controls, unreliable snapshots, random bugs, easy-to-hit limits. We needed something more serious.

Next we moved to **[E2B](https://e2b.dev/)**. Built for AI agent execution, it felt like the right step. Same architecture, just swapped the sandbox layer. We also made thread ownership explicit with a proper session table:
...
But as usage grew, we realized **the provider was not the real problem**. The real problem was our tool loop architecture. It handled short tasks but struggled with:
- Skills and MCP plumbing
- Coordination and orchestration
- Stateful session handling and retries

We were slowly building our own runtime by accident. E2B itself had friction too; persistence was beta and buggy, and the model was closer to "create and kill" than true persistence. We wanted persistence-first from day one.

## Finding the Right Sandbox

We switched to **[Daytona](https://www.daytona.io/)**. It fit because:
- Persistence-first design
- Cleaner lifecycle controls
- A natural mapping to "this thread owns a runtime"

Daytona's lifecycle settings made cleanup trivial:
...
No custom janitor needed. Cleanup shifted from our cron logic to platform lifecycle settings.
...
With the sandbox sorted, the next question was: what coding agent should run inside it?
...
The cleanest setup turned out to be **Pi over RPC**. No daemon, no internal HTTP listener, just a process. Send JSON, get JSON back.

The flow:
1. Gorkie decides execution is needed
2. Spin up or attach to the thread's Daytona sandbox
3. Start Pi in RPC mode inside a PTY
4. RPC client sends prompt and context
5. Pi runs commands and edits
6. RPC returns structured output
7. Stream progress back to Slack

The boot path was just PTY startup, `stty -echo`, and optional session resume:
...
This removed the internal HTTP server and reduced failure points. Events came back as newline-delimited JSON over the PTY stream. Simple and robust.
...
Tools became first-class instead of being pushed through an MCP proxy.
...
* Persistence-first is a different world from spin-up and spin-down
* Fewer moving parts wins almost every time
* Running an HTTP server inside your sandbox sounds cool until it is not
...
This blog post's style is inspired by [xyzeva](https://kibty.town/).
